...
public class Character {
    ...
    Character(String name, String role, int hps, int dmg, int def, int spd, String specialName, String special, PImage sprite, PImage icon, PImage sIcon, AttackType attackType, int extraW){
        ...
        this.currentHps = hps;
        this.hpPercentage= (float)currentHps/hps;
    ...
    void restore(PApplet p5){
        initAnimationSprites(p5);
        acted = false;
                ...
                if (stage == AnimationStage.WAITING) {
                    ...
                    if (clock == 5) {
                        ...
                        if (clock == 6) {
                            if (attackType == AttackType.JUMP) {
                                if (!hit) {
                                    notMissed = target.receiveDamage(dmg, p5);
                                    hit = true;
                                }
                                if (notMissed) {
                                    ...
                                    p5.textSize(20);
                                    p5.fill(0, 255, 0);
                                    p5.text("Miss!", target.x, target.y - (float) p5.width / 32);
                                    p5.popStyle();
                                }
                            } else {
                                if (!target.died) {
                        ...
                        if (clock == 6) {
                            if (attackType == AttackType.JUMP) {
                                ...
                                if (notMissed) {
                                    ...
                                    p5.textSize(20);
                                    p5.fill(0, 250, 0);
                                    p5.text("Miss!", target.x, target.y - (float) p5.width * 2 / 32);
                                    p5.popStyle();
                                }
                            } else {
                                if (!target.died) {
    ...
    boolean receiveDamage(int attack,PApplet p5){
        ...
        if(missProb > Math.random()||damage<0){
            damage = 0;
        }
        currentHps = currentHps-damage;
        if(currentHps<0){
            currentHps = 0;
        }
        hpPercentage = (float)currentHps/hps;
        if(hpPercentage == 0){
            death();
        }
        return damage!=0;
